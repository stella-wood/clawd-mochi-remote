 ...
 *     GND → GND
 *
 *   WiFi / pw: 见 secrets.h（AP_SSID / AP_PASS）  → http://192.168.4.1
 * ╚══════════════════════════════════════════════════════════════╝
 */
...
#define VIEW_CODE           2
#define VIEW_DRAW           3
#define VIEW_EYES_WINK      4
#define VIEW_EYES_SLEEP     5
#define VIEW_EYES_ANGRY     6
#define VIEW_EYES_SAD       7
#define VIEW_EYES_CUTE      8
#define VIEW_EYES_DEAD      9
...
void drawWinkEyes(bool leftClosed = true) {
  tft.fillScreen(animBgColor);
  const int16_t lx = eyeLX(0);
  const int16_t rx = eyeRX(0);
  const int16_t ey = eyeY();
  if (leftClosed) {
    tft.fillRect(lx, ey + EYE_H / 2 - 3, EYE_W, 6, C_BLACK);
    tft.fillRect(rx, ey, EYE_W, EYE_H, C_BLACK);
  } else {
...
void drawSleepZ(uint8_t step) {
  // 在右上角画Z，step控制显示几个Z
  tft.setTextColor(C_BLACK);
  if (step >= 1) { tft.setTextSize(2); tft.setCursor(175, 40); tft.print("z"); }
  if (step >= 2) { tft.setTextSize(3); tft.setCursor(190, 20); tft.print("Z"); }
}
...
void drawAngryEyes() {
  ...
  const int16_t ey = eyeY();
  // 眼睛略扁——愤怒时眯起来
  tft.fillRect(lx, ey + 10, EYE_W, EYE_H - 20, C_BLACK);
  tft.fillRect(rx, ey + 10, EYE_W, EYE_H - 20, C_BLACK);
  // 左眉毛：从外高到内低（\ 形）
  for (int8_t t = -2; t <= 2; t++) {
    tft.drawLine(lx - 4, ey - 8 + t, lx + EYE_W + 4, ey + 2 + t, C_BLACK);
  }
  // 右眉毛：从内低到外高（/ 形）
  for (int8_t t = -2; t <= 2; t++) {
    tft.drawLine(rx - 4, ey + 2 + t, rx + EYE_W + 4, ey - 8 + t, C_BLACK);
  }
}
...
void drawSadEyes() {
  ...
  // 下垂的眉毛 —— 左眉从内高到外低
  for (int8_t t = -2; t <= 2; t++) {
    tft.drawLine(lx - 4, ey - 4 + t, lx + EYE_W + 4, ey - 12 + t, C_BLACK);
  }
  // 右眉从外低到内高
  for (int8_t t = -2; t <= 2; t++) {
    tft.drawLine(rx - 4, ey - 12 + t, rx + EYE_W + 4, ey - 4 + t, C_BLACK);
  }
  // 小泪滴——左眼下方
  tft.fillCircle(lx + EYE_W / 2, ey + EYE_H + 8, 4, C_BLACK);
}

void drawCuteEyes(int16_t radius = 8) {
  tft.fillScreen(animBgColor);
  const int16_t lx = eyeLX(0) + EYE_W / 2;
  const int16_t rx = eyeRX(0) + EYE_W / 2;
  const int16_t cy = eyeCY();
  // 小圆眼睛——呆萌
  tft.fillCircle(lx, cy, radius, C_BLACK);
  tft.fillCircle(rx, cy, radius, C_BLACK);
}
...
void drawSurprisedEyes() {
  ...
  const int16_t cy = eyeCY();
  // 大圆眼睛——惊讶
  tft.fillCircle(lx, cy, 18, C_BLACK);
  tft.fillCircle(rx, cy, 18, C_BLACK);
  // 小嘴巴——张嘴惊讶
  tft.fillCircle((lx + rx) / 2, cy + 40, 8, C_BLACK);
}
...
void drawHappyEyes() {
  ...
  for (int8_t t = -3; t <= 3; t++) {
    tft.drawLine(lx - 14, cy + 4 + t, lx, cy - 10 + t, C_BLACK);
    tft.drawLine(lx, cy - 10 + t, lx + 14, cy + 4 + t, C_BLACK);
    tft.drawLine(rx - 14, cy + 4 + t, rx, cy - 10 + t, C_BLACK);
    tft.drawLine(rx, cy - 10 + t, rx + 14, cy + 4 + t, C_BLACK);
  }
}
...
void drawDeadEyes() {
  ...
  const int16_t rx = eyeRX(0) + EYE_W / 2;
  const int16_t cy = eyeCY();
  const int16_t s = 14; // X的半臂长
  // 左眼 X
  for (int8_t t = -3; t <= 3; t++) {
    tft.drawLine(lx - s, cy - s + t, lx + s, cy + s + t, C_BLACK);
    tft.drawLine(lx - s, cy + s + t, lx + s, cy - s + t, C_BLACK);
  }
  // 右眼 X
  for (int8_t t = -3; t <= 3; t++) {
    tft.drawLine(rx - s, cy - s + t, rx + s, cy + s + t, C_BLACK);
    tft.drawLine(rx - s, cy + s + t, rx + s, cy - s + t, C_BLACK);
  }
}

void drawHeart(int16_t cx, int16_t cy, int16_t sz, uint16_t col) {
  // 用两个圆+一个三角形组合成爱心
  int16_t r = sz / 2;
  tft.fillCircle(cx - r, cy - r / 2, r, col);
  tft.fillCircle(cx + r, cy - r / 2, r, col);
  tft.fillTriangle(cx - sz, cy, cx + sz, cy, cx, cy + sz + r / 2, col);
}

void drawLoveEyes(int16_t oy = 0) {
  tft.fillScreen(animBgColor);
  const int16_t lx = eyeLX(0) + EYE_W / 2;
  const int16_t rx = eyeRX(0) + EYE_W / 2;
  const int16_t cy = eyeCY() + oy;
  drawHeart(lx, cy, 22, C_BLACK);
...
void routeCmd() {
  ...
  if (cmd.length() > 0) {
    ...
    publishState(cmd, "", ok);   // 本地按键也回报，nonce 留空
  }
}
...
void routeRedraw() {
  ...
  switch (currentView) {
    ...
    case VIEW_EYES_WINK:      drawWinkEyes(true);     break;
    case VIEW_EYES_SLEEP:     drawSleepEyes(); drawSleepZ(1); drawSleepZ(2); break;
    case VIEW_EYES_ANGRY:     drawAngryEyes();        break;
    case VIEW_EYES_SAD:       drawSadEyes();          break;
    case VIEW_EYES_CUTE:      drawCuteEyes(8);        break;
...
// ═════════════════════════════════════════════════════════════
//  STATE REPORT — 执行完往回报一句，让云端知道屏幕真的动了
// ═════════════════════════════════════════════════════════════
void publishState(const String& cmd, const String& nonce, bool ok) {
  ...
  payload += (ok ? "1" : "0");
  payload += ",\"view\":";
  payload += String(currentView);
  payload += ",\"up\":";
  payload += String(millis());
  payload += "}";

  // retain = true：云端随时能读到"最后一次执行了什么"
  mqtt.publish("clawd/mochi/state", payload.c_str(), true);
...
// ═════════════════════════════════════════════════════════════
//  REPORT — 执行完直接告诉 Worker（EMQX Serverless 不给读 retained）
// ═════════════════════════════════════════════════════════════

// ⚠️ 这两行的值在 secrets.h 里填
const char* REPORT_URL   = SECRET_REPORT_URL;
const char* REPORT_TOKEN = SECRET_REPORT_TOKEN;

void reportState(const String& cmd, const String& nonce, bool ok) {
  if (WiFi.status() != WL_CONNECTED) return;
  if (nonce.length() == 0) return;   // 本地按键不上报，省流量

  WiFiClientSecure httpsClient;
  httpsClient.setInsecure();

  HTTPClient http;
  if (!http.begin(httpsClient, REPORT_URL)) {
    Serial.println("上报失败: begin");
    return;
  }
  http.addHeader("Content-Type", "application/json");
  http.addHeader("X-Report-Token", REPORT_TOKEN);
  ...
  Serial.print("上报 Worker: HTTP ");
  Serial.println(code);        // 200=成功  403=token不一致  其他看情况
  http.end();
}
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (hash >= 0) {
    cmd   = msg.substring(0, hash);
    nonce = msg.substring(hash + 1);
  }

  bool ok = executeCommand(cmd);
  if (!ok) {
    Serial.print("未知 MQTT 指令: ");
    Serial.println(cmd);
  }

  publishState(cmd, nonce, ok);
  //reportState(cmd, nonce, ok);
}
...
void mqttReconnect() {
  ...
  if (mqtt.connect("clawd_mochi_01", MQTT_USER, MQTT_PASS)) {
    Serial.println(" 已连接！");
    mqtt.subscribe("clawd/mochi/cmd");
    Serial.println("已订阅 clawd/mochi/cmd");
  } else {
    Serial.print("失败，状态码=");
...
if (WiFi.status() == WL_CONNECTED) {
  ...
  Serial.println("家用 WiFi 连接失败，本地热点仍可使用");
}
...
mqtt.setCallback(mqttCallback);
  ...
  tft.fillRect(0, 0, DISP_W, 4, C_ORANGE);
  tft.setTextColor(C_WHITE);  tft.setTextSize(2);
  tft.setCursor(12, 16);  tft.print(String("WiFi: ") + AP_SSID);
  tft.setTextColor(C_MUTED);  tft.setTextSize(1);
  tft.setCursor(12, 44);  tft.print(String("password: ") + AP_PASS);
  tft.setTextColor(C_WHITE);  tft.setTextSize(2);
